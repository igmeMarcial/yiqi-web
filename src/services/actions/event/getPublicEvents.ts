...
export async function getPublicEvents(filters: {
  ...
  // Obtener las ubicaciones únicas de los eventos
  ...
  const locationsList = locations.map(event => event.location)

  // Retornar los eventos y las ubicaciones
  return {
    events: events.map(event =>
      PublicEventSchema.parse({
        ...event,
        registrations: event.registrations.length // Contamos las inscripciones
      })
    ),
