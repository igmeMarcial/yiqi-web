...
import { Mail, Phone, Briefcase, Building, Twitter, Linkedin, Instagram, Globe, User } from 'lucide-react';
    ...
    <div className="min-h-screen bg-black text-gray-100 py-12 px-4 sm:px-6 lg:px-8 font-sans">
      <div className="max-w-3xl mx-auto">
          ...
          <div className="relative px-4 sm:px-6 lg:px-8 pb-8">
            <div className="flex flex-col sm:flex-row items-center">
              ...
              <div className="mt-16 sm:mt-0 sm:ml-32 text-center sm:text-left">
                ...
                <p className="mt-1 text-xl text-[#00E5BE]">{user.position}</p>
                <p className="mt-2 text-sm text-gray-400">{user.company}</p>
              </div>
            </div>
            <div className="mt-6 grid grid-cols-1 sm:grid-cols-2 gap-4">
              {user.privacySettings.email && (
        ...
        <div className="mt-8 bg-[#0A0A0A] rounded-lg shadow-lg p-6 border border-[#1A1A1A]">
          ...
          <p className="text-gray-300 leading-relaxed">{user.shortDescription}</p>
        </div>

        <div className="mt-8 bg-[#0A0A0A] rounded-lg shadow-lg p-6 border border-[#1A1A1A]">
          <h2 className="text-2xl font-semibold mb-4 text-white">Enlaces</h2>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            {user.privacySettings.website && (
              <a href={user.website} target="_blank" rel="noopener noreferrer" className="flex items-center text-[#00E5BE] hover:underline">
                <Globe className="w-5 h-5 mr-2" />
                {translations.es.website}
              </a>
            )}
            {user.privacySettings.linkedin && (
              <a href={user.linkedin} target="_blank" rel="noopener noreferrer" className="flex items-center text-[#00E5BE] hover:underline">
                <Linkedin className="w-5 h-5 mr-2" />
                {translations.es.linkedin}
              </a>
            )}
            {user.privacySettings.x && (
              <a href={user.x} target="_blank" rel="noopener noreferrer" className="flex items-center text-[#00E5BE] hover:underline">
                <Twitter className="w-5 h-5 mr-2" />
                {translations.es.x}
              </a>
            )}
            <a href={user.instagram} target="_blank" rel="noopener noreferrer" className="flex items-center text-[#00E5BE] hover:underline">
